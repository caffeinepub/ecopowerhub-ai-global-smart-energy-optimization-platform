...
actor EcoPowerHubAI {
  ...
  public query func transform(input : OutCall.TransformationInput) : async OutCall.TransformationOutput {
    // This function is called by the IC infrastructure for HTTP outcall transformations
    // It must remain public and without authorization checks to function properly
    OutCall.transform(input);
  };

  // Energy Snapshot - requires authenticated user access since it aggregates all energy data
  public query ({ caller }) func getEnergySnapshot() : async EnergySnapshot {
    if (Principal.isAnonymous(caller)) {
      Debug.trap("Unauthorized: Anonymous principal not allowed");
    };
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Debug.trap("Unauthorized: Only users can view the energy snapshot");
    };

    // Filter out entries with non-positive energy consumption (considered inactive devices)
    let activeEnergyData = Iter.toArray(
      Iter.filter(
        textMap.vals(energyData),
        func(d : EnergyData) : Bool { d.energyConsumption > 0.0 },
      )
    );

    // Calculate total power consumption by summing energyConsumption of all active devices
    var totalPower : Float = 0.0;
    for (data in activeEnergyData.vals()) {
      totalPower += data.energyConsumption;
    };

    // Count the number of active devices (positive energyConsumption)
    let activeDevices = activeEnergyData.size();

    // Calculate average carbon intensity (average energyConsumption) across all active devices
    let averageCarbon = if (activeDevices == 0) {
      0.0; // No devices, default to 0.0
    } else {
      var totalCarbon : Float = 0.0;
      for (data in activeEnergyData.vals()) {
        totalCarbon += data.energyConsumption;
      };
      totalCarbon / Float.fromInt(activeDevices);
    };

    let cycleCost : Nat = 0; // No specific cycle cost state variable found

    // Use current IC time as the timestamp for the snapshot
