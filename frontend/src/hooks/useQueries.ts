...
// Energy Snapshot Query
export function useGetEnergySnapshot() {
  const { actor, isFetching } = useActor();

  return useQuery<EnergySnapshot>({
    queryKey: ['energySnapshot'],
    queryFn: async () => {
      if (!actor) throw new Error('Actor not available');
      return actor.getEnergySnapshot();
    },
    enabled: !!actor && !isFetching,
...
export function useDeleteEVOAuthConfig() {
  ...
  return useMutation({
    mutationFn: async (_provider: EVProvider) => {
      if (!actor) throw new Error('Actor not available');
